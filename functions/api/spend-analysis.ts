...
async function querySpendAnalysis(startDate: string, endDate: string, env: Env) {
  const [weeklyTrend, channelBreakdown, declineRows, periodSummaryRows] = await Promise.all([
    // Weekly spend trend
    runQuery(
      `WITH
      card_unblocked AS (
        SELECT DISTINCT f9_dw005_loc_acct AS loc_acct FROM ${TABLES.principal_card_updates}
        WHERE f9_dw005_1st_unblk_all_mtd_tms IS NOT NULL AND TRIM(CAST(f9_dw005_1st_unblk_all_mtd_tms AS STRING)) != ''
          AND f9_dw005_hce_txn_ind LIKE '%0%' AND f9_dw005_net_txn_ind LIKE '%0%' AND fx_dw005_contc_less_flg LIKE '%Y%' AND f9_dw005_contc_txn_ind LIKE '%0%'
      ),
      weekly_eligible AS (
    ...
    // Channel breakdown
    runQuery(
      `SELECT CASE WHEN fx_dw007_txn_typ='RA' AND fx_dw007_rte_dest='L' THEN 'QRIS' WHEN fx_dw007_txn_typ='TM' THEN 'Online' ELSE 'Offline' END AS channel,
        COUNT(*) AS txn_count, ROUND(SUM(f9_dw007_amt_req/100),0) AS spend_idr, COUNT(DISTINCT f9_dw007_prin_crn) AS unique_cards
      FROM ${TABLES.authorized_transaction}
      WHERE f9_dw007_dt BETWEEN @startDate AND @endDate AND (fx_dw007_stat IS NULL OR TRIM(fx_dw007_stat)='' OR fx_dw007_stat=' ') AND fx_dw007_txn_typ NOT IN ('PM','BE','RF')
      GROUP BY channel`,
      { startDate, endDate }, env,
    ),
    // Decline breakdown
    runQuery<{ code: string; cnt: number; amount_idr: number }>(
      `SELECT fx_dw007_stat AS code, COUNT(*) AS cnt, ROUND(SUM(f9_dw007_amt_req/100),0) AS amount_idr
      FROM ${TABLES.authorized_transaction}
      WHERE f9_dw007_dt BETWEEN @startDate AND @endDate AND fx_dw007_stat IS NOT NULL AND TRIM(fx_dw007_stat)!='' AND fx_dw007_stat!=' '
