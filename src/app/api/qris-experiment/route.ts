...
export async function GET(request: Request) {
  try {
    ...
    const [cohortComparison, merchantBreakdown, merchantGrowth, mixedMerchantStats] =
      await Promise.all([
    ...
    return NextResponse.json({ cohortComparison, merchantBreakdown, merchantGrowth, mixedMerchantStats });
  } catch (error) {
